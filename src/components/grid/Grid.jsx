...
export function GridVertical({ item, children }) {
  return (
    <>
      <div
        className={`grid bg-white grid-cols-1 px-4 pt-4 xl:grid-cols-${item.cols} xl:gap-4 dark:bg-gray-900`}
      >
        {children}
      </div>
    </>
...
export function GridHorizontal({ item, children }) {
  return (
    <>
      <div
        className={`grid bg-white gap-4 px-4 pt-4 md:grid-cols-2 xl:grid-cols-${item.cols}`}
      >
        {children}
      </div>
    </>
